...
## Contributing
To contribute clone the project then run the following make command:

```bash
make init-project
```

This will configure everything in your project, and you can begin developing.

In general, the only file that should be modified is api/openapi.yaml. The rest of the files are autogenerated, so there is no need to modify them directly. If there is a need to change a project file, it should be modified via the templated located in the `out` directory. See the OpenAPI Generator [documentation](https://openapi-generator.tech/docs/templating) for details.

After making changes to the api/openapi.yaml document, execute `make codegen` and files will be regenerated.
